...
The graph can be partitioned in 2 ways. Either the graph is padded so that it can be evenly divisible by the number of threads or METIS [Karypis] can be used to partition the graph.\

\textbf{Boundary Nodes}\
When a graph is divided into subgraphs, some of the vertices found in the subgraph will have neighbors only in the subgraph while others will have some which are inside while others are outside. The latter are referred to as boundary nodes and they are the ones who might have color conflicts. For many graphs, especially if METIS is not used and the partitions are small, the number of boundary nodes is roughly equal to the total number of vertices. However, it can help speedup in some case and when heuristics like MIN and MAX are used, we need to know the number of neighbors of a vertex which is outside the subgraph.\\
