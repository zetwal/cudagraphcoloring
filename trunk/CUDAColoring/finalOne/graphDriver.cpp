...
int main(int argc, char *argv[]){  
	if (argc != 7){
		...
		cout << "7 Arguments needed: " << endl 
		<< "cudaExe <passes> <atificial (1 => artificial file)>  <metis (1 => use metis)>  <randomness for GPU (0-2)> <CPU technique (1 => SDO)> <GPU technique (0: FF, 1:SDO, 2:Max, 3:Min)" << endl << "e.g. cudaExe 1 0 0 0 1 1" << endl;
	...
	for (int repeatGPU=0; repeatGPU<GPUIterations; repeatGPU++)
	{
		...
		//cudaGraphColoring(adjacentList, boundaryList, graphColors, numOutside, conflictTmp, boundaryCount, 
		//				  maxDegree, graphSize, passes, _gridSize*_blockSize, _gridSize, _blockSize,
		//				  startPartitionList, endPartitionList, randomList, numRandoms, GPUSDO);
		...
		if (sdoConflictSolver == true)
			//conflictSolveSDO(adjacentList, conflict, conflictCount, graphColors, numOutside, graphSize, maxDegree);
		...
		numColorsParallels[repeatGPU] = numColorsParallel;
